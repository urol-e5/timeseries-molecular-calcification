...
```{r exon-variability}
# Function to calculate exon variability by timepoint
calculate_exon_variability <- function(exon_df, biomin_gene_ids, species_code) {
  # Filter to biomin genes
  exon_biomin <- exon_df %>%
    filter(gene_id %in% biomin_gene_ids)
  ...
  # Get sample columns
  sample_cols <- names(exon_df)[!names(exon_df) %in% c("gene_id", "e_id", "chr", "strand", "start", "end")]
  ...
  exon_cv_by_tp <- exon_long %>%
    group_by(gene_id, timepoint, sample_id) %>%
    summarise(
      n_exons = n(),
...
apul_exon_cv <- calculate_exon_variability(apul_exon_counts, biomin_genes$apul, "apul")
peve_exon_cv <- calculate_exon_variability(peve_exon_counts, biomin_genes$peve, "peve")
ptua_exon_cv <- calculate_exon_variability(ptua_exon_counts, biomin_genes$ptua, "ptua")

exon_cv_all <- bind_rows(apul_exon_cv, peve_exon_cv, ptua_exon_cv) %>%
  mutate(species = species_lookup[species_code])

cat("Exon variability calculated for:\n")
exon_cv_all %>%
  group_by(species_code) %>%
...
exon_cv_wide <- exon_cv_all %>%
  select(species_code, species, gene_id, timepoint, mean_cv_exon) %>%
...
gbm_by_tp <- biomin_meth %>%
  filter(!is.na(methylation)) %>%
  group_by(species_code, species, gene_id_clean, timepoint) %>%
  summarise(
...
gbm_wide <- gbm_by_tp %>%
  select(species_code, species, gene_id_clean, timepoint, mean_meth) %>%
...
# Calculate stress timepoint exon variability
...
```{r test-h3}
# Merge with GBM change data
h3_data <- high_var_genes %>%
  inner_join(gbm_analysis, by = c("species_code", "gene_id" = "gene_id_clean")) %>%
  mutate(
...
h3_tests <- h3_data %>%
  group_by(species_code) %>%
  summarise(
    wilcox_p = wilcox.test(abs_gbm_change ~ high_var)$p.value,
    high_var_mean_change = mean(abs_gbm_change[high_var], na.rm = TRUE),
    low_var_mean_change = mean(abs_gbm_change[!high_var], na.rm = TRUE),
    .groups = "drop"
  ) %>%
  mutate(
    h3_supported = high_var_mean_change > low_var_mean_change,
    interpretation = ifelse(h3_supported, 
                           "SUPPORTED: High variability genes show more differential GBM",
                           "NOT SUPPORTED: No difference in GBM changes")
  )
...
# Create summary data for species comparison
species_summary <- tibble(
  species = rep(species_lookup, each = 3),
  hypothesis = rep(c("H1", "H2", "H3"), 3),
  p_value = c(
    h1_tests$wilcox_p[h1_tests$species_code == "apul"],
    h2_focus$t_test_p[h2_focus$species_code == "apul"],
    h3_tests$wilcox_p[h3_tests$species_code == "apul"],
    h1_tests$wilcox_p[h1_tests$species_code == "peve"],
    h2_focus$t_test_p[h2_focus$species_code == "peve"],
    h3_tests$wilcox_p[h3_tests$species_code == "peve"],
    h1_tests$wilcox_p[h1_tests$species_code == "ptua"],
    h2_focus$t_test_p[h2_focus$species_code == "ptua"],
    h3_tests$wilcox_p[h3_tests$species_code == "ptua"]
  ),
  supported = c(
    h1_tests$h1_supported[h1_tests$species_code == "apul"],
    h2_focus$h2_supported[h2_focus$species_code == "apul"],
    h3_tests$h3_supported[h3_tests$species_code == "apul"],
    h1_tests$h1_supported[h1_tests$species_code == "peve"],
    h2_focus$h2_supported[h2_focus$species_code == "peve"],
    h3_tests$h3_supported[h3_tests$species_code == "peve"],
    h1_tests$h1_supported[h1_tests$species_code == "ptua"],
    h2_focus$h2_supported[h2_focus$species_code == "ptua"],
    h3_tests$h3_supported[h3_tests$species_code == "ptua"]
  )
) %>%
  mutate(
    ...
    result = ifelse(supported, "Supported", "Not Supported")
  )
...
p_summary <- ggplot(species_summary, aes(x = hypothesis, y = species, fill = result)) +
  ...
  scale_fill_manual(values = c("Supported" = "#00A087", "Not Supported" = "#E64B35"),
                    name = "Hypothesis") +
  labs(
